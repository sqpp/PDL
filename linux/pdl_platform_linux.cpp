/*
 * Native Linux implementation of the PDL platform API.
...
static char s_config_decrypt_key[256] = {0};

void pdl_platform_set_pocsag_decrypt_key(const char *key)
{
	if (!key) { s_config_decrypt_key[0] = '\0'; return; }
	strncpy(s_config_decrypt_key, key, sizeof(s_config_decrypt_key) - 1);
	s_config_decrypt_key[sizeof(s_config_decrypt_key) - 1] = '\0';
}

const char *pdl_platform_pocsag_decrypt_key(void)
{
	const char *k = getenv("PDL_POCSAG_DECRYPT_KEY");
	if (!k || !k[0])
		k = getenv("PDW_POCSAG_DECRYPT_KEY"); /* legacy env name */
	if (k && k[0]) return k;
	return s_config_decrypt_key[0] ? s_config_decrypt_key : NULL;
}
