...
{
#ifdef __linux__
	strcpy(szOSType, "Linux");
	nOSType = OS_UNKNOWN;
	if (szOS) strcpy(szOS, szOSType);
	return nOSType;
#else
	nOSType = OS_UNKNOWN ;
			...
			case VER_PLATFORM_WIN32_NT :
				if(OSVersionInfo.dwMajorVersion > 4) {
					if(OSVersionInfo.dwMinorVersion > 0) {
						nOSType = OS_WINXP ;
						wsprintf(szOSType, TEXT("WINXP Version %ld.%02ld %s Build %u"), OSVersionInfo.dwMajorVersion, OSVersionInfo.dwMinorVersion,
								OSVersionInfo.szCSDVersion, OSVersionInfo.dwBuildNumber & 0xFFFF) ;
					}
					else {				
						nOSType = OS_WIN2000 ;
