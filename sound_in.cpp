...
void pdl_linux_feed_audio(char *lpAudioBuffer, long LenAudioBuffer)
{
	...
	if (flex_timer) {
		...
		if (flex_timer == 0) {
			bMode_IDLE = true;
			if (!pocbit) { BaudRate = 1600; config_index=INDEX1600; display_showmo(MODE_IDLE); }
		}
	} else if (mb.timer) {
		mb.timer--;
...
#ifdef _WIN32
	waveInReset(hWaveIn);
	if (!(waveInClose(hWaveIn))) return(FALSE);
	free_audio_buffers();
#endif
	buffers_ready = 0;
	last_buff_processed = -1;
...
//   Callback_Function (Windows only)
...
#else
	(void)hwi;(void)uMsg;(void)dwInstance;(void)dwParam1;(void)dwParam2;
#endif
}
